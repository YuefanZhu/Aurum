...
import csv
inpt = int(input("Please enter starting date in 'YYMMDD' format (091201 is default and earliest date):"))
...
for i in index:
    if int(mystring[(i+num+4):(i+num+6)]+mystring[(i+num):(i+num+4)])>=inpt:
...
temp = [next_weekday(date[-1],1).strftime('%m/%d/%Y'),'']
x1 = urllib.request.urlopen(url1)
mystring1 = x1.read().decode('utf-8').split("\n")
for j in range(len(mystring1)):
    if "Code-088691" in mystring1[j]:
...
with open('C:\\output.csv', 'w', newline='') as csvfile:
